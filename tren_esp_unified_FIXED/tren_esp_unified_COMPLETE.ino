...
void loop_deadband_experiment() {
    if (flag_deadband == false) {
        ...
        Serial.println("  Increasing PWM until motion detected...");
    }

    // Read current distance
    read_ToF_sensor();
    double current_distance = medi;
    ...
    // Check for motion
    if (distance_change >= motion_threshold && MotorSpeed > 0) {
        // Motion detected!
    ...
    // No motion yet - continue increasing PWM
    if (abs(initial_distance - current_distance) < motion_threshold) {
